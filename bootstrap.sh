...
elif [[ -e /etc/almalinux-release || -e /etc/rocky-release || -e /etc/centos-release || -e /etc/fedora-release ]]; then
	os="centos"
	os_version=$(grep -shoE '[0-9]+' /etc/almalinux-release /etc/rocky-release /etc/centos-release /etc/fedora-release | head -1)
...
if [ -d "$HOME/ansible-vpn" ]; then
  pushd $HOME/ansible-vpn
  git pull
  popd
else
  git clone https://github.com/notthebee/ansible-vpn $HOME/ansible-vpn
fi
...
set -e
cd $HOME/ansible-vpn
[ -d $HOME/ansible-vpn/.venv ] || $PYTHON -m venv .venv
export VIRTUAL_ENV="$HOME/ansible-vpn/.venv"
export PATH="$HOME/ansible-vpn/.venv/bin:$PATH"
...
cd $HOME/ansible-vpn && ansible-galaxy install --force -r requirements.yml
...
touch $HOME/ansible-vpn/custom.yml

custom_filled=$(awk -v RS="" '/username/&&/dns_nameservers/&&/root_host/{print FILENAME}' $HOME/ansible-vpn/custom.yml)
...
if [[ "$custom_filled" =~ "custom.yml" ]]; then
  ...
  echo
  cd $HOME/ansible-vpn && ansible-playbook --ask-vault-pass run.yml
  exit 0
fi

clear
echo "Welcome to ansible-vpn!"
echo
...
echo "username: \"${username}\"" >> $HOME/ansible-vpn/custom.yml
...
if [[ "$adguard_enable" =~ ^[yY]$ ]]; then
  echo "enable_adguard_unbound_doh: true" >> $HOME/ansible-vpn/custom.yml
fi
...
echo "root_host: \"${root_host}\"" >> $HOME/ansible-vpn/custom.yml
...
echo "dns_nameservers: \"${dns_nameservers}\"" >> $HOME/ansible-vpn/custom.yml
...
if [[ ! $AWS_EC2 =~ true ]]; then
  ...
  echo "enable_ssh_keygen: true" >> $HOME/ansible-vpn/custom.yml

  if [[ "$new_ssh_key_pair" =~ ^[yY]$ ]]; then
    echo
    read -p "Please enter your SSH public key: " ssh_key_pair

    echo "ssh_public_key: \"${ssh_key_pair}\"" >> $HOME/ansible-vpn/custom.yml
  fi
fi
...
if [[ "$email_setup" =~ ^[yY]$ ]]; then
  ...
  if [ ! -z ${email} ]; then
    echo "email: \"${email}\"" >> $HOME/ansible-vpn/custom.yml
  fi

  read -p "'To' e-mail [${email_login}]: " email_recipient
  if [ ! -z ${email_recipient} ]; then
    echo "email_recipient: \"${email_recipient}\"" >> $HOME/ansible-vpn/custom.yml
  fi

  echo "email_smtp_host: \"${email_smtp_host}\"" >> $HOME/ansible-vpn/custom.yml
  echo "email_smtp_port: \"${email_smtp_port}\"" >> $HOME/ansible-vpn/custom.yml
  echo "email_login: \"${email_login}\"" >> $HOME/ansible-vpn/custom.yml
fi


# Set secure permissions for the Vault file
touch $HOME/ansible-vpn/secret.yml
chmod 600 $HOME/ansible-vpn/secret.yml

if [ -z ${email_password+x} ]; then
  echo
else 
  echo "email_password: \"${email_password}\"" >> $HOME/ansible-vpn/secret.yml
fi

if [[ $user_password =~ '"' ]]; then
  echo "user_password: '${user_password}'" >> $HOME/ansible-vpn/secret.yml
else
  echo "user_password: \"${user_password}\"" >> $HOME/ansible-vpn/secret.yml
fi
...
echo "jwt_secret: ${jwt_secret}" >> $HOME/ansible-vpn/secret.yml
echo "session_secret: ${session_secret}" >> $HOME/ansible-vpn/secret.yml
echo "storage_encryption_key: ${storage_encryption_key}" >> $HOME/ansible-vpn/secret.yml

echo
echo "Encrypting the variables"
ansible-vault encrypt $HOME/ansible-vpn/secret.yml
...
if [[ "$launch_playbook" =~ ^[yY]$ ]]; then
  if [[ $EUID -ne 0 ]]; then
    ...
    cd $HOME/ansible-vpn && ansible-playbook --ask-vault-pass -K run.yml
  else
    cd $HOME/ansible-vpn && ansible-playbook --ask-vault-pass run.yml
  fi
else
  echo "You can run the playbook by executing the bootstrap script again:"
  echo "cd ~/ansible-vpn && bash bootstrap.sh"
  exit
fi
